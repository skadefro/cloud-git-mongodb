 ...
 */
  ...
  savefile(object) {
    ...
    const folder = os.tmpdir() + "/" + sha.substring(0, 2);
  ...
  getfile(sha) {
    ...
    const filename = os.tmpdir() + "/" + sha.substring(0, 2) + "/" + sha.substring(2);
 ...
 */
  ...
  async getUploadPack(shas, haves = null, skipparents = false) {
    const closure = {};

    await this.computeClosure(closure, shas, haves, skipparents);
  ...
  async computeClosure(closure, queue, haves = null, skipparents = false) {
