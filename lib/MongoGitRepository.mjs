...
export class MongoGitRepository extends GitRepository {
  ...
  */
  constructor(db, collectionname, repoName) {
    ...
    this.zlib = true;
    this.treeIndexCollection = this.db.collection(this.bucketName + "_treeIndex");
  }

  /**
   * Get the tree index for a given SHA
   * @param {string} sha
   * @returns {Promise<any>}
   */
  async getTreeIndex(sha) {
    return this.treeIndexCollection.findOne({ sha });
  }

  /**
   * Set the tree index for a given SHA
   * @param {string} sha
   * @param {any} indexData
   * @returns {Promise<void>}
   */
  async setTreeIndex(sha, indexData) {
    await this.treeIndexCollection.updateOne(
      { sha },
      { $set: { sha, indexData } },
      { upsert: true }
    );
  ...
  async DeleteRepo() {
    ...
    await this.db.dropCollection(this.bucketName + "_treeIndex");
 ...
 */
  ...
  async receivePack(req, commands, objects) {
    ...
    try {
      ...
      for (objectCount = 0; objectCount < objects.length; objectCount++) {
        const object = objects[objectCount];
        ...
        if (this.uploadasync) {
          await this.storeObject(object, updatedoc).catch((e) => {
  ...
  async getObjectMeta(sha) {
    ...
    if (file == null) {
      file = await this.db.collection(this.bucketName + ".files").findOne({ "metadata.sha": `${sha}` });
    }
    if (file == null) {
  ...
  async upsertRef(ref, sha) {
    ...
    if (this.headref == null) {
      ...
      for (let i = 0; i < refs.length; i++) {
        if (refs[i].ref == `refs/heads/master` || refs[i].ref == `refs/heads/main` || refs[i].ref == `refs/heads/16384`) {
