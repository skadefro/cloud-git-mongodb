 ...
 */
  ...
  async receivePack(req, commands, objects) {
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      if (tools.isZeroId(command.destId)) {
        ...
        await this.collection.updateOne(
          { repo: this.repoName, ref: command.ref },
          {
            $set: {
              repo: this.repoName,
              ref: command.ref,
              name: command.ref + " " + command.destId,
              sha: command.destId,
              _type: "hash",
              _modified: _modified
            },
    ...
    if (this.headref) {
      ...
      await this.collection.updateOne(
        ...
        { 
          $set: { repo: this.repoName, name: "HEAD " + this.headref, headref: this.headref, ref: "HEAD", sha: headsha, _type: "hash", _modified },
          $setOnInsert: {
            _acl: this._acl,
            _created: _created
          }
  ...
  async getObject(req, sha) {
    try {
      ...
      if (file.metadata.zlib == true) {
        buffer = inflate.inflate(buffer);
      }
      const data = Buffer.from(buffer);
      tools.debug("GET OBJECT", sha, file.metadata.objecttype, data.length, "bytes");
      const object = {
        sha,
        objectType: file.metadata.objecttype,
        data
      };
      console.log("sha", sha, "objecttype", file.metadata.objecttype, "data", data.length, "bytes read");
      console.log("object", data.toString("utf8"));
      return object;
    } catch (error) {
  ...
  async createAnnotatedTag(branch, tagName, tagger, message) {
    ...
    await this.collection.updateOne(
      { repo: this.repoName, ref: tagRef },
      { $set: { repo: this.repoName, ref: tagRef, sha: tagObject.sha, _type: 'hash' } },
      { upsert: true }
    );
    this.headref = null;
 ...
 */
  async createLightweightTag(branch, tagName) {
    ...
    await this.collection.updateOne(
      { repo: this.repoName, ref: tagRef },
      { $set: { repo: this.repoName, ref: tagRef, sha: commitSha, _type: 'hash' } },
      { upsert: true }
    );
