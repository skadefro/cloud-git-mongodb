 ...
 */
    ...
    try {
      ...
      while(objects.length > 0) {
        ...
        for (const object of batch) {
          ...
          //   throw new Error(`Object ${object.sha.toString("hex")} data is empty`);
          // }
          if(object.sha == "a5f2cf23c26b9e64386774f7bbaf9a921320d2c2" || object.sha.toString("hex") == "a5f2cf23c26b9e64386774f7bbaf9a921320d2c2") {
            var b = true;
          }
          if(object.data == null) {
            object.data = Buffer.alloc(0);
          }
  ...
  async getObjectMeta(sha) {
    ...
    if (normalCollectionObject) {
      if(normalCollectionObject.data == null) {
        normalCollectionObject.data = Buffer.alloc(0);
      }
      const object = {
        sha,
        objectType: normalCollectionObject.objecttype,
        size: normalCollectionObject.data.length
      };
    ...
    const file = await this.db.collection(this.bucketName + ".files").findOne({ "filename": `blob_${sha}` });
    if (!file) {
      throw new Error(`Blob with sha1 ${sha} was not found`);
    }
  ...
  async storeObject(object, metadata = {}) {
    ...
    // Check if the object is stored in GridFS
    const existingFile = await this.db.collection(this.bucketName + ".files").findOne({ "filename": `blob_${sha}` });
    if (existingFile != null) {
      tools.debug("UPLOAD", "sha", sha, tools.ObjectTypes[object.objectType], "SKIPPED already exists in GridFS");
    ...
    if (object.data.length < 2 * 1024 * 1024) { // Less than 2MB
      ...
      tools.debug("UPLOAD", "sha", sha, tools.ObjectTypes[object.objectType], "STORED in normal collection");
      return sha;
    } else { // 2MB or larger
      ...
      tools.debug("UPLOAD", "sha", sha, tools.ObjectTypes[object.objectType], "STORED in GridFS");
 ...
 */
  ...
  async getObjectMetaBatch(shas) {
    ...
    const normalCollectionMap = new Map();
    normalCollectionObjects.forEach(obj => {
      normalCollectionMap.set(obj.sha, {
        sha: obj.sha,
    ...
    // Check if the remaining objects are stored in GridFS
    const remainingShas = shas.filter(sha => !normalCollectionMap.has(sha));
    const files = await this.db.collection(this.bucketName + ".files").find({
      "metadata.sha": { $in: remainingShas }
    }).toArray();
    ...
    // Merge normal collection objects into the metadata map
    normalCollectionMap.forEach((value, key) => {
