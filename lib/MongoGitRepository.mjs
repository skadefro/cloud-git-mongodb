 ...
 */
  ...
  async preProcessObject(object, metadata) {
    if (object.objectType == 1 || object.objectType == "commit") {
      ...
      metadata.related_shas = [metadata.tree].concat(metadata.parents);
    }
 ...
 */
  ...
  async getUploadPack(shas, haves = null, skipparents = false, filters = []) {
    ...
    if(shas.length == 0 && haves.length == 0) {
      ...
      const resultshas = await state.repository.collection.distinct("sha", {});
    ...
    if (shas != null && shas.length > 0) {
      ...
      if(filters.length == 0) {
        console.time("aggregate shas");
        const totalShas = shas.length;
        let resultShas = new Set();
        
        for (let i = 0; i < totalShas; i += BATCH_SIZE) {
                ...
                },
                {
                    $graphLookup: {
                        ...
                        restrictSearchWithMatch: {
                            objecttype: { $in: [1, 2, 3] } // Restrict to relevant object types if possible
                        }
                    }
                },
                {
            ...
            // console.time(`aggregate batch ${i / BATCH_SIZE}`);
            const results = await this.collection.aggregate(pipeline).toArray();
            results.forEach(doc => {
                resultShas.add(doc.initial_sha);
                doc.all_related_document_shas.forEach(sha => resultShas.add(sha));
            });
            // console.timeEnd(`aggregate batch ${i / BATCH_SIZE}`);
        }
        
        // make Unique
        var resultshas = Array.from(resultShas).concat(shas).filter((v, i, a) => a.indexOf(v) === i);
        console.log("Unique SHA count:", resultShas.size);
        console.log("Unique SHA count:", resultshas.length);
        console.timeEnd("aggregate shas");

        // fs.writeFileSync("shas.json", JSON.stringify(resultshas, null, 2));
      ...
      let resultShas = new Set();
      const conditions = [];
      // combine filters if blop and or tree needs to be excluded
      if (filters != null && filters.length > 0) {
        for(let i = 0; i < filters.length; i++) {
          if (filters[i].split(":")[0] == "blob" && filters[i].split(":")[1] == "none") {
            conditions.push({ objecttype: { $ne: 3 } });
          }
          if (filters[i].split(":")[0] == "tree" && filters[i].split(":")[1] == "none") {
            conditions.push({ objecttype: { $ne: 2 } });       
          }
        }
      }
          ...
          const pipeline = [
              ...
              },
              {
                  $graphLookup: {
                      ...
                      as: "all_related_documents",
                      maxDepth: MAX_DEPTH,
                      restrictSearchWithMatch: matchConditions
                  }
              },
              {
    ...
    await this.computeClosure(closure, shas, haves, skipparents, filters);
