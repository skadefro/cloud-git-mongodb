      ...
      while(objects.length > 0) {
        ...
        for (const object of batch) {
          ...
          if (object.objectType == 1 || object.objectType == "commit") {
            ...
            postprocess.push({ sha: object.sha.toString("hex"), tree: commit.tree});
          }
          if (object.objectType == 4 || object.objectType == "tag") {
            const tag = tools.parseTag(object);
            updatedoc.tree = tag.object;
            postprocess.push({ sha: object.sha.toString("hex"), tree: tag.object});
          }
          if (object.objectType == 2 || object.objectType == "tree") {
            const entries = tools.parseTree(object);
            updatedoc.entries = entries;
            postprocess.push({ sha: object.sha.toString("hex"), entries: entries});
          }
 ...
 */
   ...
   * @returns {Promise<{sha: string, objectType: string, size: number, data: Buffer}[]>} Array of objects with metadata
   */
