 ...
 */
  ...
  async receivePack(req, commands, objects) {
    ...
    try {
      ...
      const _createdby = req?.user ? req.user.username : "unknown";
      const _createdbyid = req?.user ? req.user._id : "unknown";
      ...
      tools.logMemoryUsage("after heads parsed", starttime);
      ...
      while (objects.length > 0) {
        ...
        for (const object of batch) {
          objectCount++;
          const updatedoc = { repo: this.repoName, name: object.objectType + " " + object.sha.toString("hex"), sha: object.sha.toString("hex"), _type: tools.ObjectTypes[object.objectType], objecttype: object.objectType, _acl: this._acl, _created, _createdby, _createdbyid, _modified };
        ...
        if (objectCount % 1000 == 0 && objectCount > 0) {
          ...
          tools.logMemoryUsage(`objects objectCount ( ${Math.round(msperobject)}ms per object )`, starttime);
        }
      }
    } finally {
  ...
  async getObjectMeta(sha) {
    ...
    const normalCollectionObject = await this.collection.findOne({ sha }, { projection: { objecttype: 1, size: 1, parents: 1, tree: 1, entries: 1 } });
 ...
 */
  ...
  async handleUploadPack(resultShas, batch, objecttypes, haves) {
    const commits = await this.collection.find({ sha: { $in: batch }, objecttype: 1 }).toArray();
    const batchShas = commits.map(commit => commit.sha);
    let getshas = commits.map(commit => commit.parents).flat().filter(sha => !haves.includes(sha));
  ...
  async getUploadPack(shas, haves = null, filters = []) {
    if (shas == null) shas = [];
    if (haves == null) haves = [];
    ...
    let refs = (await this.getRefs()).filter(x => x.ref != "HEAD");
    // git cli will ask for all, including tags
    let getall = refs.length == shas.length;
    if(getall == true) {
      for(let i = 0; i < refs.length; i++) {
        if(!shas.includes(refs[i].sha)) {
          getall = false;
          break;
        }
      }
    ...
    if(getall == false) {
      // Since tags (and brances for that sake) can overlab with sha's, lets also compare unique sha's
      refs = refs.filter((v, i, a) => a.findIndex(t => (t.sha === v.sha)) === i);
      shas = shas.filter((v, i, a) => a.indexOf(v) === i);

      getall = refs.length == shas.length;
      if(getall == true) {
        for(let i = 0; i < refs.length; i++) {
          if(!shas.includes(refs[i].sha)) {
            getall = false;
    ...
    if(getall == false) {
      const branches = refs.filter(x=> x.ref.startsWith("refs/heads/"));
      getall = branches.length == shas.length;
      if(getall == true) {
        for(let i = 0; i < branches.length; i++) {
          if(!shas.includes(branches[i].sha)) {
            getall = false;
            break;
          }
        }
    ...
    if ((shas.length == 0 && haves.length == 0) || getall == true) {
      const resultshas = await this.collection.distinct("sha", {objecttype: { $in: objecttypes }});
    ...
    do {
      ...
      let promises = [];
      for(let i = getshas.length - 1; i >= 0; i -= tools.batchSize) {
        let batch = getshas.splice(i - tools.batchSize + 1, tools.batchSize);

        promises.push(this.handleUploadPack(resultShas, batch, objecttypes, haves));
      ...
      getshas = getshas.concat(_results.flat());
      getshas = getshas.filter((v, i, a) => a.indexOf(v) === i);
      tools.logMemoryUsage(`Promise.all ${resultShas.size} shas, ${getshas.length} in queue`, starttime);
  ...
  async computeClosure(closure, queue, haves = null, filters = []) {
  ...
  async processCommit(closure, queue, object, haves = null) {
