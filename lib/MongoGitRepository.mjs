 ...
 */
  ...
  async getUploadPack(shas, excludeParents = false) {
    const closure = {};

    await this.computeClosure(closure, shas, excludeParents);
  ...
  async computeClosure(closure, queue, excludeParents) {
      ...
      await Promise.all(batch.map(async sha => {
        return new Promise(async (resolve, reject) => {
          ...
          if(object.children != null && this.generateChildren == true) {
            object.children.forEach(child => this.enqueue(closure, queue, child));
            if(object.parents != null) {
              object.parents.forEach(parent => this.enqueue(closure, queue, parent));
            }
          } else 
          if (object.objectType === "commit" || object.objectType === 1) {
            await this.processCommit(closure, queue, object, excludeParents);
  ...
  async processCommit(closure, queue, object, excludeParents) {
    if (object.parents != null || object.tree != null) {
      tools.debug("commit", object.tree);
      this.enqueue(closure, queue, object.tree);
      if (object.parents && object.parents.length > 0 && excludeParents == false) {
        object.parents.forEach(parent => this.enqueue(closure, queue, parent));
      ...
      this.enqueue(closure, queue, commit.tree);
      if (commit.parents && commit.parents.length > 0 && excludeParents == false) {
        commit.parents.forEach(parent => this.enqueue(closure, queue, parent));
      }
    }
  }
