 ...
 */
  ...
  async getUploadPack(shas) {
    const closure = {};

    await this.computeClosure(closure, shas);
  ...
  async computeClosure(closure, queue) {
      ...
      await Promise.all(batch.map(async sha => {
        return new Promise(async (resolve, reject) => {
          ...
          if (object.objectType === "commit" || object.objectType === 1) {
            await this.processCommit(closure, queue, object);
  ...
  async processCommit(closure, queue, object) {
