      ...
      while(objects.length > 0) {
        ...
        for (const object of batch) {
          ...
          if (object.objectType == 1 || object.objectType == "commit") {
            ...
            updatedoc.parentshas = [commit.tree].concat(commit.parents)
          }
          if (object.objectType == 2 || object.objectType == "tree") {
            const tree = tools.parseTree(object);
            updatedoc.tree = tree;
            updatedoc.parentshas = tree.map(x => x.sha)
          }
          updatedoc.zlib = this.zlib;
          

          if (updatedoc.size < 2 * 1024 * 1024) { // Less than 2MB
            const dataToStore = this.zlib ? deflate.deflate(object.data) : object.data;
