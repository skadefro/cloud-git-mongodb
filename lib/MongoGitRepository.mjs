  ...
  async receivePack(req, commands, objects) {
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      if (this.isZeroId(command.destId)) {
        Protocol.debug("DELETE", "ref", command.ref);
        await this.collection.deleteOne({ repo: this.repoName, ref: command.ref })
      } else {
        Protocol.debug("UPDATE", "ref", command.ref, "sha", command.destId);
    ...
    for (let i = 0; i < objects.length; i++) {
      ...
      if(this.uploadasync) {
        this.storeObject(object, updatedoc).then(()=> {
          Protocol.debug("UPLOAD", "sha", object.sha.toString("hex"));
        }).catch((e)=> {
    ...
    if(this.headref == null) {
      if (refs.find(x => x.ref == "HEAD") == null) {
        ...
        if (this.headref) {
          Protocol.debug("UPDATE", "ref", "HEAD", "sha", headsha, "headref", this.headref);
