...
function trimBuffer(state) {
  ...
  state.buffer = state.buffer.slice(state.start);
  state.bufferLength -= state.start;
  state.start = 0;
}
...
class GitProtocolTransform extends Transform {
  ...
  async processStages() {
    ...
    do {
      if(this.state.packObjectsLeft == 9400 || this.state.packObjectsLeft == 9401) {
        var b = true;
      }
      ...
      console.log("processStages", this.state.stage, "redo", (bufferlength != postBufferlength));
    } while (bufferlength != postBufferlength);
    tools.logMemoryUsage(`processStages ${this.state.stage} commands ${this.state.commands.length} objects ${Object.keys(this.state.objects).length}`);
  }
