...
const SupportedCapabilities = "side-band-64k delete-refs report-status filter allow-reachable-sha1-in-want";
  ...
  try {
    ...
    if (state.repository.getUploadPack != null && state.repository.collection != null) {
      if (haves.length == 0 && (state.filters == null || state.filters.length == 0)) {
        ...
        shas = await state.repository.getUploadPack(wants, haves, false, state.filters);
      } else {
        const havecount = haves.length;
        tools.debug(`Traverse ${haves.length} has shas`);
        if(haves.length > 0) {
          haves = await state.repository.getUploadPack(haves, null, true, state.filters);
        }
...
async function readObject(state) {
  ...
  if (state.packObjectsLeft % 100 == 0) {
    tools.logMemoryUsage(`readObject: Received ${tools.ObjectTypes[objectType]} object ${state.packObjectsLeft}/${state.numObjects}, skipcounter: ${state.skipcounter}`, "handlePost");
  }
  ...
  if (state.service == "git-receive-pack") {
    ...
    if (command === "want" || command === "have") {
      ...
      for(var i = 0; i < state.filters.length; i++) {
        ...
        if(f.split(":")[0] != "blob") throw new Error(`Protocol error. Unsupported filter: ${f} (only blob is supported)`);
      }
...
export async function handlePost(repository, service, req, res) {
  try {
    ...
    tools.logMemoryUsage("handlePost " + service, "handlePost");
    ...
    try {
      ...
      tools.logMemoryUsage(service, "handlePost");
...
class GitProtocolTransform extends Transform {
  ...
  async _transform(chunk, encoding, callback) {
    try {
      if (this.state.bufferLength + chunk.length > this.state.buffer.length) {
        ...
        if (currentsize > 0) {
          tools.logMemoryUsage(`expanded buffer from ${tools.formatBytes(currentsize)} to ${tools.formatBytes(this.state.buffer.length)}`, "handlePost");
        }
      }
  ...
  async processStages() {
    ...
    // tools.logMemoryUsage(`processStages ${this.state.stage} commands ${this.state.commands.length} objects ${this.state.numObjects}`, "handlePost");
  }
