...
async function sendWantedObjects(req, res, state) {
  try {
    ...
    const shas = await state.repository.db.collection(state.repository.bucketName + ".files").distinct("metadata.sha", {  });
    const objects = await state.repository.getObjectMetaBatch(shas);

    // const shas = await state.repository.db.collection(state.repository.bucketName + ".files").distinct("metadata.sha", {  });
    // const missing = []
    // for(let i = 0; i < shas.length; i++) {
    //   if(!objects.find(x => x.sha == shas[i])) {
    //     missing.push(shas[i]);
    //   }
    // }
