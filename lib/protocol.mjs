...
async function sendWantedObjects(req, res, state) {
  try {
    ...
    // @ts-ignore
    if (state.repository.getObjectMetaBatch != null && state.repository.collection != null && alwayssendallsha == true) {
      const shas = await state.repository.collection.distinct("sha", {});
