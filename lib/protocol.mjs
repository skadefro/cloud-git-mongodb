...
async function sendWantedObjects(req, res, state) {
  try {
    ...
    if (state.repository.getObjectMetaBatch != null && state.repository.collection != null ) {
      ...
      const refs = await state.repository.getRefs();
      refs.forEach(ref => {
        if (shas.indexOf(ref.sha) == -1) {
          allsha = false;
        }
      });
      allsha = false;
    }
    if (state.repository.getObjectMetaBatch != null && state.repository.collection != null && allsha == true) {
      
      // @ts-ignore
      if (allsha == true || alwayssendallsha == true) {
        shas = await state.repository.collection.distinct("sha", {});
        tools.debug(`sending all ${shas.length } shas`);
      // } else {
      //   // @ts-ignore
      //   const shasInDb = await state.repository.collection.distinct("sha", { sha: { $in: shas } });
      //   shas = shasInDb;
      //   tools.debug(`Traverse ${shas.length } wanted shas`);
