...
async function processTree(req, state, closure, queue, object) {
  if (object.tree) {
    object.tree.forEach(item => enqueue(closure, queue, item.sha));
    return;
  }
  if (!object.data) {
    await state.repository.getObjectData(object);
...
async function computeClosure(req, state, closure, queue) {
  while (queue.length > 0) {
    ...
    // console.timeLog("sendWantedObjects", "computed", Object.keys(closure).length, "objects", queue.length, "in queue");
  }
}
...
async function sendWantedObjects(req, res, state) {
  try {
    ...
    // @ts-ignore
    if (state.repository.getObjectMetaBatch != null && state.repository.collection != null) {

      let shas = state.commands.filter(x => x.command === "want").map(x => x.sha);
      tools.debug("wanted shas", shas);

      // @ts-ignore
      if (alwayssendallsha == true) {
        shas = await state.repository.collection.distinct("sha", {});
        tools.debug("sending all shas", shas);
      } else {
        const lookup = await state.repository.collection.aggregate([
          {
            $match: {
              sha: { $in: shas }
            }
          ...
          {
            $graphLookup: {
              from: state.repository.bucketName,
              startWith: "$parentshas",
              connectFromField: "parentshas",
              connectToField: "sha",
              as: "allParentDocs"
            }
          },
          {
            $unwind: "$allParentDocs"
          },
          {

            $group: {
              _id: null,
              allUniqueSHAs: { $addToSet: "$allParentDocs.sha" }
            }
          },
          {
        ...
        shas = shas.concat(lookup[0].allUniqueSHAs);
        // distinct shas
        shas = [...new Set(shas)];
        tools.debug("sending all shas", shas);
        tools.logMemoryUsage(`sendWantedObjects ${shas.length} shas`, "sendWantedObjects");
      }


      // console.timeLog("sendWantedObjects", "upload-pack", shas.length, "objects");
      ...
      let objectCount = 0;
      const cursor = state.repository.collection.find({});
      for await (const object of cursor) {
        if (!object.data) {
          ...
          // console.timeLog("sendWantedObjects", "DOWNLOADED", object.sha);
        ...
        if (objectCount % 5000 === 0) {
          const msperobject = (Date.now() - starttime) / objectCount;
          // console.timeLog("sendWantedObjects", "sent", objectCount, "objects", "(" + msperobject.toFixed(2) + "ms per object)", object.sha);
    ...
    // console.timeLog("sendWantedObjects", "upload-pack", objects.length, "objects");
    ...
    while (objects.length > 0) {
      ...
      await Promise.all(batch.map(async object => {
        if (!object.data) {
          console.log("download ", object.sha);
          await state.repository.getObjectData(object);
        }
        ...
        if (objectCount % 1000 === 0) {
          const msperobject = (Date.now() - starttime) / objectCount;
          // console.timeLog("sendWantedObjects", "sent", objectCount, "objects", "(" + msperobject.toFixed(2) + "ms per object)");
          tools.logMemoryUsage(`sent ${objectCount} objects (${msperobject.toFixed(2)}ms per object)`, "sendWantedObjects");
        }
      }));
    }
