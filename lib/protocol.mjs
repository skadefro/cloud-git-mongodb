...
import { isZeroId, ZeroIdStr, Zero, PackHeader, FlushPkt, ZeroId, LF, DataBand, ProgressBand, ErrorBand, debug, Stages, ObjectTypes, ObjectNames } from './tools.mjs';

import { parseCommit, parseTree } from './tools.mjs';
...
// rm -rf test1 && git clone http://localhost:3000/git/test1
...
async function computeClosure(req, state, closure, queue, sha) {
  let object = closure[sha];
  if (object == null) {
    object = await state.repository.getObject(req, sha);
    closure[sha] = object;
  }
  // const object = (closure[sha] = await state.repository.getObject(req, sha));
  var b = true;
  if (object.objectType === "commit" || object.objectType === 1) {
    const commit = parseCommit(object);
    console.log("commit", commit.tree);
    await enqueue(closure, queue, commit.tree);
    if (commit.parent != null) {
      console.log("commit", commit.parent);
      await enqueue(closure, queue, commit.parent);
    }
    // const msg = object.data.toString().split("\n");
    // let n = 0;
    // while (msg[n] !== "") {
    //   let match = msg[n++].match(/\s*(tree|parent)\s+(.+)/);
    //   console.log("commit", match[2]);
    //   // match && enqueue(closure, queue, match[2]);
    // }
  } else if (object.objectType === "tree" || object.objectType === 2) {
    const tree = await parseTree(object);
    for (let i = 0; i < tree.length; i++) {
      await enqueue(closure, queue, tree[i].sha);
    }
    // let offset = 0;
    // while (offset < object.data.length) {
    //   if (object.data[offset] === 0) {
    //     enqueue(closure, queue, object.data.subarray(offset + 1, offset + 21).toString("hex"));
    //     offset += 21;
    //   } else {
    //     offset++;
    //   }
...
async function getUploadPack(req, state) {
  ...
  while (queue.length > 0) {
    var b = true;
    await computeClosure(req, state, closure, queue, queue.shift());
    var b = true;
  }
...
async function sendWantedObjects(req, res, state) {
  let objects;
  objects = await getUploadPack(req, state);
  ...
  objects.forEach((object) => {
    ...
    if (typeof type === "string") {
      type = ObjectNames[object.objectType];
    }
    ...
    res.write(toPktLinesWithBand(DataBand, Buffer.from(newheader), data));
...
async function readObject(state) {
  ...
  if (!{ 1: 1, 2: 1, 3: 1, 4: 1, 7: 1 }[objectType]) {
    ...
    if (objectType === 0) {
      ...
      if (state.commands.length > 0) {
        state.stage = Stages.PackChecksum;
        return false;
      }
      console.log("Received objectType 0, which is unexpected. Skipping processing for this object.");
    }
    throw new Error(
  ...
  console.log(`#${state.packObjectsLeft} readObjectHeader: initial byte: ${byte.toString(16)}, type (4 bits): ${objectType}, length (4 bits): ${length}, bufferOffset: ${bufferOffset}`);
  ...
  if (objectType === 7) {
    ...
    console.log("Get baseObject with sha", baseSha);
    if (baseSha == null || baseSha.trim() == "") {
      console.log("baseSha is null or empty");
  ...
  // @ts-ignore
  console.log(`slice buffer: bufferOffset: ${bufferOffset}, bytesWritten: ${data.engine.bytesWritten}`);
  // @ts-ignore
  ...
  console.log("Store object with sha", sha1);
...
// function writePktLine(socket, line, band) {
//   const bandLength = band ? 1 : 0;
//   const length = Buffer.byteLength(line) + 4 + bandLength;
//   const lengthHex = length.toString(16).padStart(4, '0');
//   const data = Buffer.from(line);
//   const dataWithBand = band ? Buffer.concat([band, data]) : data;
//   const packet = Buffer.concat([Buffer.from(lengthHex, 'utf8'), dataWithBand]);
//   debug(`Sending pkt-line with band ${band ? band.toString('hex') : 'none'}:`, packet.toString('utf8'), 'Length:', length);
//   socket.write(packet);
// }

function flushPkt(socket) {
  socket.write(FlushPkt);
}
...
function readPktLine(state) {
  ...
  console.log(`readPktLine: length: ${length}, line: ${line}`);
  ...
  if (state.service == "git-receive-pack") {
    ...
    if (isZeroId(srcId) && !isZeroId(destId)) {
      command = "create";
    } else if (!isZeroId(srcId) && !isZeroId(destId)) {
      command = "update";
    } else if (!isZeroId(srcId) && isZeroId(destId)) {
      command = "delete";
    }
...
function objectSha(data, type) {
  let header = Buffer.from(`${ObjectTypes[type]} ${data.length}\0`);
...
export async function handleGetRefs(repository, req, res, next) {
  try {
    ...
    if (!refs || refs.length === 0) {
      console.log("write ref", ZeroId, " capabilities^{}", Zero, _caps);
      res.write(toPktLine(ZeroId, " capabilities^{}", Zero, _caps, LF));
    } else {
      console.log("write ref", refs[0].sha, refs[0].ref);
      let first = true;
      for (let i = 0; i < refs.length; i++) {
        if (refs[i].ref == null || refs[i].sha == null) continue;
        if(first) {
          console.log("write ref", refs[i].sha, refs[i].ref, _caps);
          res.write(toPktLine(refs[i].sha, " ", refs[i].ref, Zero, _caps, LF));
          first = false;
        } else {
          console.log("write ref", refs[i].sha, refs[i].ref);
    ...
    debug('Error handling GetRefs:', error.message);
    if (!res.headersSent) {
      res.writeHead(500);
    }
...
export async function handlePost(repository, service, req, res) {
  try {
    ...
    try {
      // git-receive-pack - SEND objects to client
      // git-upload-pack  - RECEIVE objects from client
      ...
      while (state.stage !== Stages.Final) {
        switch (state.stage) {
          ...
          case Stages.PackData: {
            // // https://git-scm.com/docs/pack-format
            await readObject(state);
            break;
          }
          case Stages.PackChecksum: {
      ...
      if (service === "git-upload-pack") {
        ...
        if (state.requestedCapabilities != null && state.requestedCapabilities.includes("report-status")) {
          res.write(toPktLinesWithBand(DataBand, toPktLine("unpack ok\n")));
          // @ts-ignore
          ...
          for (const ref of refs) {
            res.write(toPktLinesWithBand(DataBand, toPktLine(`ok ${ref}\n`)));
          }
          res.write(toPktLinesWithBand(DataBand, FlushPkt));
        }
      }
      ...
      debug('Error handling POST:', error.message);
      if (!res.headersSent) {
        res.writeHead(500);
      }
      res.end('Internal Server Error');
