 ...
 */
export async function parseTree(repo, tree) {
  if (tree.objectType != "tree" && tree.objectType != 2) throw new Error(`${tree.sha} is not a tree. (${tree.objectType})`)
  const treeIndex = await repo.getTreeIndex(tree.sha);
  if (treeIndex) {
    return treeIndex.indexData;
  }
  ...
  function parseTreeData(treedata) {
    ...
    while (offset < treedata.length) {
      let spaceIndex = treedata.indexOf(32, offset);
  ...
  if (treedata[0] === 0x78 && treedata[1] === 0x9c) { // Check for zlib header
    try {
      const decompressedData = Buffer.from(inflate.inflate(treedata));
      const entries = parseTreeData(decompressedData);
      await repo.setTreeIndex(tree.sha, entries);
      return entries;
    } catch (error) {
      console.error("Failed to decompress tree data:", error);
      throw new Error("Failed to decompress tree data");
    }
  } else {
    const entries = parseTreeData(treedata);
    await repo.setTreeIndex(tree.sha, entries);
    return entries;
  }
}
