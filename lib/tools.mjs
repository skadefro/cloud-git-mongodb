...
export const logMemoryUsage = (label, timelabel) => {
  ...
  if(timelabel != null && timelabel != "") {
    try {
      console.timeLog(timelabel, label, `\tRSS: ${formatBytes(memoryUsage.rss)} Heap Used: ${formatBytes(memoryUsage.heapUsed)} Heap Total: ${formatBytes(memoryUsage.heapTotal)}`);
    } catch (error) {
      console.log(`Memory usage after ${label}:`, `\tRSS: ${formatBytes(memoryUsage.rss)} Heap Used: ${formatBytes(memoryUsage.heapUsed)} Heap Total: ${formatBytes(memoryUsage.heapTotal)}`);
    }
  } else {
    console.log(`Memory usage after ${label}:`, `\tRSS: ${formatBytes(memoryUsage.rss)} Heap Used: ${formatBytes(memoryUsage.heapUsed)} Heap Total: ${formatBytes(memoryUsage.heapTotal)}`);
  }
