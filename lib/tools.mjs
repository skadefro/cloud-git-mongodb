...
export function createTree(entries) {
  ...
  function appendSlashIfDir(entry) {
    ...
    return (entry.mode === '040000'.toString(8)) ? entry.name + '/' : entry.name
  }
  var test = entries.sort(compareTreeEntryPath)

  entries.forEach(x => console.log(x.mode.toString(8), "blob", x.sha, x.name,));
  console.log("test")
  test.forEach(x => console.log(x.mode.toString(8), "blob", x.sha, x.name,));
