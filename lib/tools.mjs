...
export function objectSha(data, type) {
 ...
 */
export function createTag({ object, type, tagger, message }) {
  // Validate inputs
  ...
  const sha = objectSha(data, 4)
  // Convert tag data to Buffer
  return {
    objectType: 4, // 'tag',
    data, 
    sha
...
export function createCommit({ tree, parent, author, committer, message }) {
  ...
  // Calculate the SHA-1 hash of the commit data
  const sha = objectSha(data, 1)
